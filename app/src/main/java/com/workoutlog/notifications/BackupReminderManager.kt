...
class BackupReminderManager @Inject constructor(
    ...
    fun scheduleReminder(settings: BackupReminderSettings) {
        val delay = when (settings.frequency) {
            BackupReminderFrequency.DAILY   -> dailyDelay(settings.hour, settings.minute)
            BackupReminderFrequency.WEEKLY  -> weeklyDelay(settings.dayOfWeek, settings.hour, settings.minute)
            BackupReminderFrequency.MONTHLY -> monthlyDelay(settings.monthlyOption, settings.hour, settings.minute)
        }
    ...
    private fun monthlyDelay(option: BackupMonthlyOption, hour: Int, minute: Int): Long {
        val now = LocalDateTime.now()
